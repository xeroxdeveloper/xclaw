...
export async function noteSecurityWarnings(cfg: OpenClawConfig) {
  ...
  const saferRemoteAccessLines = isXClawMode()
    ? [
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Исправьте: запустите ${formatCliCommand(isXClawMode() ? "xclaw configure" : "openclaw configure")} для установки пароля`,
              `  Или переключитесь на токен: ${formatCliCommand(isXClawMode() ? "xclaw config set gateway.auth.mode token" : "openclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Исправьте: запустите ${formatCliCommand(isXClawMode() ? "xclaw doctor --fix" : "openclaw doctor --fix")} для генерации токена`,
              `  Или установите напрямую: ${formatCliCommand(
                isXClawMode() ? "xclaw config set gateway.auth.mode token" : "openclaw config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        isXClawMode() 
        ...
        `  Исправьте: ${formatCliCommand(isXClawMode() ? "xclaw config set gateway.bind loopback" : "openclaw config set gateway.bind loopback")}`,
  ...
  const lines = warnings.length > 0 
    ? warnings 
    : [isXClawMode() ? "- Проблем с безопасностью каналов не обнаружено." : "- No channel security warnings detected."];
