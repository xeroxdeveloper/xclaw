...
export async function doctorCommand(
  ...
  intro(isXClawMode() ? "Доктор XClaw" : "OpenClaw doctor");
  ...
  if (resolveMode(cfg) === "local" && sourceConfigValid) {
    ...
    if (needsToken) {
      note(
        ...
        isXClawMode() ? "Доступ к шлюзу" : "Gateway auth",
      );
        ...
        options.generateGatewayToken === true
          ...
          : options.nonInteractive === true
            ...
            : await prompter.confirmRepair({
                message: isXClawMode() ? "Сгенерировать и настроить токен шлюза прямо сейчас?" : "Generate and configure a gateway token now?",
                initialValue: true,
              });
      if (shouldSetToken) {
        ...
        note(isXClawMode() ? "Токен шлюза настроен." : "Gateway token configured.", isXClawMode() ? "Доступ к шлюзу" : "Gateway auth");
      }
    }
  }
  ...
  if (legacyState.preview.length > 0) {
    note(legacyState.preview.join("\n"), isXClawMode() ? "Обнаружено устаревшее состояние" : "Legacy state detected");
    const migrate =
      options.nonInteractive === true
        ? true
        : await prompter.confirm({
            message: isXClawMode() ? "Перенести устаревшее состояние (сессии/агенты) прямо сейчас?" : "Migrate legacy state (sessions/agent/WhatsApp auth) now?",
            initialValue: true,
          });
    if (migrate) {
      ...
      if (migrated.warnings.length > 0) {
        note(migrated.warnings.join("\n"), isXClawMode() ? "Предупреждения доктора" : "Doctor warnings");
      }
    }
  }
  ...
  if (shouldWriteConfig) {
    ...
    if (fs.existsSync(backupPath)) {
      runtime.log(`${isXClawMode() ? "Бэкап" : "Backup"}: ${shortenHomePath(backupPath)}`);
    }
  ...
  if (options.workspaceSuggestions !== false) {
    ...
    if (await shouldSuggestMemorySystem(workspaceDir)) {
      note(MEMORY_SYSTEM_PROMPT, isXClawMode() ? "Рабочая область" : "Workspace");
    }
  }

  const finalSnapshot = await readConfigFileSnapshot();
  if (finalSnapshot.exists && !finalSnapshot.valid) {
    runtime.error(isXClawMode() ? "Невалидный конфиг:" : "Invalid config:");
  ...
  outro(isXClawMode() ? "Доктор завершил проверку." : "Doctor complete.");
}
